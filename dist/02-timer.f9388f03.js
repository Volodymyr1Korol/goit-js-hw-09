...
const options = {
    ...
    onClose (selectedDates) {
        if (selectedDates[0].getTime() <= Date.now()) (0, _notiflixNotifyAio.Notify).failure("Please choose a date in the future");
        else {
...
class Timer {
    ...
    //*  запустили таймер
    startTimer() {
        if (refs.startBtn.hasAttribute("isActive")) return;
        this.timerId = setInterval(()=>{
            const currentTime = new Date();
            const deltaTime = selectedTime - currentTime;
            const componentsTimer = convertMs(deltaTime);
            //*  остановили таймер очистили интервал
            if (deltaTime <= 1000) this.stopTimer();
