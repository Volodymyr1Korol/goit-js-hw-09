...
function onFormSubmit(e) {
    e.preventDefault();
    let delay1 = Number(formEl.elements.delay.value);
    const step = Number(formEl.elements.step.value);
    const amount = Number(formEl.elements.amount.value);
    for(let i = 0; i < amount; i += 1){
        createPromise(i + 1, delay1).then(({ position , delay  })=>(0, _notiflixDefault.default).Notify.success(`✅ Fulfilled promise ${position} in ${delay}ms`, {
                position: "right-bottom"
            })).catch(({ position , delay  })=>(0, _notiflixDefault.default).Notify.failure(`❌ Rejected promise ${position} in ${delay}ms`, {
                position: "right-bottom"
            }));
        delay1 += step;
    }
